...
reg1 <- felm(complete ~ capacity+ bf_covid + year| month, 
             data = pp_data)
(reg1 %>% summary)
...
mod_stargazer("../report/reg_3_0.text", reg_3, type = "text", align = T, style = "aer")
```
...
coeffi3_plot$bf_covid = "1"
grep("bf_covid0", coeffi3_plot$rownames) %>% as.vector()
coeffi3_plot$bf_covid[c(grep("bf_covid0", coeffi3_plot$rownames) %>% as.vector())] <- "0" 

coeffi3_plot$Estimate = coeffi3_plot$Estimate %>% round(3)
coeffi3_plot = coeffi3_plot %>% mutate(bf_covid = ifelse(bf_covid == 1, "before_covid", "after_covid"))
```
...
ggsave(filename = "../report/p_coef .png", plot = p_coef, width = 12)
```
```{r, warning =F, message =F}
coeffi3_plot2 <- coeffi3_plot %>% select(bf_covid, big5, type,Estimate)
diff_plot <- {}
# coeffi3_plot2
...
diff_plot$dif_non5 <- c(34.514, -4.445, -1.045,  4.515)
...
ggsave(filename = "../report/p_coef_dif.png", plot = p_coef_dif, width = 12)
```

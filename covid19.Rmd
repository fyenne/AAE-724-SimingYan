...
comp.mean = (pp_data %>% group_by(yearmon, area3) %>% summarise(mean(complete)) %>% ungroup())
pp_data_test <- merge(comp.mean, pp_data, by = c("yearmon", "area3"))
names(pp_data_test)[3] = "mean.com"
#--------------------------------------------
...
which(pp_data_dummy$big_five == 1) %>% length
pp_data_dummy$complete[which(pp_data_dummy$big_five == 1)] %>% sum()
# 2664.8/2766
# 0.96341
pp_data_dummy$complete[which(pp_data_dummy$big_five != 1)] %>% sum()
...
compar_big5 <- data.frame(big5 = c("1","0") %>% as.factor(), avg_com = c(0.96341, 0.59136), n = c(2766, 9563))
...
comp_big5_plot <- gpt(compar_big5) + aes(x = big5) +
  geom_bar(aes(weights = avg_com, fill = avg_com %>% as.factor())) +
  geom_boxplot(aes(x = big5, y = n/9000, color = n %>% as.factor()), alpha = I(.3)) +
  geom_text(data = compar_big5,
            ...
            size = 9,
            vjust = 1) +
  labs(fill = "average_completed per month", 
       color = "total amount",
       title = "Comparation of big-five and non-big-five owned power plant") +
  theme(text = element_text(size = 22),
        plot.title = element_text(size = 25))+
  xlab("is big_five comapny owned?") +
  ylab("") 
# comp_big5_plot

ggsave("comp_big5_plot.png", plot = comp_big5_plot, width = 12)
```
...
# v
# felm(complete ~ big_five + spr_festival+bf_covid|area3, data = pp_data_dummy) %>% summary()
# 
...
reg2 <- lm(complete ~ big_five + big_five:spr_festival + big_five:bf_covid + as.factor(yearmon) + area3, data = pp_data_dummy)
(reg2 %>% summary)$coef %>% data.frame() %>% view

reg_covid <- felm(complete ~ big_five*spr_festival + big_five*bf_covid + 
                    big_five*bf_covid*Hydro + big_five*bf_covid*wind + 
                    big_five*bf_covid*`Fossil-fuel` + big_five*bf_covid*photovoltaic|area3, data = pp_data_dummy)
            # omit=c("big_five0:spr_festival1","big_five1:bf_covid1"))
#--------------------------------------------

mod_stargazer("reg_covid.text", reg_covid, type = "text")
...
reg_covid_month <- felm(complete ~ big_five + big_five + 
                    ...
                    big_five*`Fossil-fuel` + big_five*photovoltaic|area3 + yearmon, data = pp_data_dummy)
reg_covid_month %>% stg
```

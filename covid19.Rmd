...
mmmmodift <- gsub("plant_1_", "", names(pp_data_after_dummy));mmmmodift 
...
# setdiff(names(pp_data_after_dummy), names(pp_test)) #0
# c = setdiff(names(pp_test), names(pp_data_after_dummy));c #144
...
data_funny <- cbind(pp_data_after_dummy4, predict_19_20_afternn)
...
dfpp <- data_funny %>% group_by(month) %>% summarise(sum(predict_19_20_afternn))

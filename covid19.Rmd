...
reg1 <- felm(complete ~ capacity + big_five + city + bf_covid|year + month + area3, 
             data = pp_data)
(reg1 %>% summary)$coef
...
pp_data %>% str
reg_covid <- felm(complete ~ 
                    capacity +
                    city +
                    big_five +
                    bf_covid + 
                    big_five:bf_covid:type_of_p|
                    area3 + year + month|
                    0|
                    plant + area3 + year , data = pp_data)
            # omit=c("big_five0:spr_festival1","big_five1:bf_covid1"))
#--------------------------------------------
reg_covid %>% summary
mod_stargazer("../report/reg_3.text", reg_covid, type = "text", align = T, style = "aer")
...
pp_data_bf_dummy <- pp_data_bf_dummy[,!names(pp_data_bf_dummy) %in% c("month","year","parent_company", "area", "plant_1" )]
...
nn <- neuralnet(f, 
          data = pp_train, hidden = c(5), algorithm = "backprop", linear.output = T, learningrate = 0.0001)

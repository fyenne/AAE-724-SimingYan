...
df_big5_year_comp
gpt(df_big5_year_comp %>% filter(year == 20))+geom_line(aes(x = yearmon, y = `mean.com`, color = big_five))
```
#--------------------------------------------
#### 3.5 plots of big5 
...
```{r}
pp_data$complete = pp_data$complete*100

write.csv( pp_data, file = "pp_data_real1.csv")
...
compar_big5 <- data.frame(big5 = c("1","0") %>% as.factor(), 
                          avg_com = c(9644.1, 5861.7), 
                          avg_cap = c(29.725, 21.11), 
                          avg_com.nHydro = c(9644.1,5861.7), 
...
### 4.01 serious regs
```{r}
pp_data2 <- read.csv("./pp_data_real1.csv", encoding = "utf-8")
pp_data2 <- pp_data2[,-c(1,2,3, 4)]
# pp_data %>% head
# covid_sx_monthly <- covid_sx_monthly[-7,]
covid_sx_monthly$year = 20
names(covid_sx_monthly)[2] = "new_case"

pp_data2 <- mutate(pp_data2, bf_covid = ifelse(year != 20 | year == 20 & month == 1, "1", "0"))
pp_data2 <- mutate(pp_data2, af_covid = ifelse(year == 20 & month > 4, "1", "0"))
# pp_data2$new_case = "0"

covid_sx_monthly
# pp_data2 <- 
# pp_data2 %>% str
# pp_data2 <- mutate(pp_data2, new_case = ifelse(year == 20, "", 0))
pp_data_2 <- merge(pp_data2, covid_sx_monthly, by = c("year", "month"), all = T)
pp_data_2$new_case[which(is.na(pp_data_2$new_case == T))] = 0

# pp_data_2 %>% group_by(yearmonth) %>% summarise(mean(as.numeric(af_covid)))
# pp_data_2$complete = pp_data_2$complete*100
write.csv(pp_data_2, "./real_pp_data_2.csv", row.names = F)
pp_data_2$big_five = as.factor(pp_data_2$big_five)
# pp_data_2 %>% group_by(yearmonth) %>%  summarise(mean(new_case))
pp_data_2 <- mutate(pp_data_2, on_covid = ifelse(bf_covid == "1", "0", "1"))

# which(as.numeric(pp_data_2$on_covid) + as.numeric(pp_data_2$bf_covid) != 1)
```
...
reg1 <- felm(complete ~ capacity +
               on_covid + year + new_case| month, 
             data = pp_lag_combine)
(reg1 %>% summary)
...
```{r}
# pp_data[which(pp_data$Hydro == "1"), ] %>% View()

coeffi3_plot <- na.omit(coeffi3_plot)
coeffi3_plot$rownames <- rownames(coeffi3_plot)
grep("big_five0", coeffi3_plot$rownames) %>% as.vector() #3  5  7  8 10 12 13
coeffi3_plot$big5 = "1"
coeffi3_plot$big5[c(grep("big_five0", coeffi3_plot$rownames) %>% as.vector())] <- "0" 
# coeffi3_plot <- coeffi3_plot[-c(1:2),]

coeffi3_plot$type <- gsub(".{1,100}type_of_p", "", coeffi3_plot$rownames)
# coeffi3_plot$type[1:2] <- "hydro"
# coeffi3_plot$big5[1:2] <- '0'
#--------------------------------------------

coeffi3_plot$on_covid = "0"
grep("on_covid1", coeffi3_plot$rownames) %>% as.vector()
coeffi3_plot$on_covid[c(grep("on_covid1", coeffi3_plot$rownames) %>% as.vector())] <- "1" 

coeffi3_plot$Estimate = coeffi3_plot$Estimate %>% round(3)
coeffi3_plot = coeffi3_plot %>% mutate(on_covid = ifelse(on_covid == 1, "during_covid", "before_covid"))

#--------------------------------------------
# baseline for hydro, draft of 8.
coeffi3_plot <- coeffi3_plot[-(1:3),]
coeffi3_plot[14,] <- coeffi3_plot[5,]
coeffi3_plot[14,1] = 0
coeffi3_plot[14,8] = "during_covid"
```

```{r}
# ggthemr('light',type = 'outer')
# coeffi3_plot$type
p_coef <- ggplot(coeffi3_plot) + geom_col(position = "dodge") +
  aes(x = big5, y = Estimate, group = type, fill = type) +
  geom_text(aes(x = big5, y = Estimate, group = type, label = Estimate),
            position = position_dodge2(width = 1 , preserve = "single"),
            vjust = -.1,
            size = 7) + 
  facet_grid(rows = vars(on_covid),
             scales = "free",
             space = "free")+  
  theme(text = element_text(size =20),
        plot.title = element_text(size = 25),
        axis.text.x = element_text(angle = 0)) + 
  labs(title  = "Coefficients value of model (3)",
       x = "Ownership type of certain plants",
       y = "Estimated electricity generation amount")
  
 p_coef
  
ggsave(filename = "../report/p_coef_d8 .png", plot = p_coef, width = 12)
```
```{r, warning =F, message =F}
coeffi3_plot2 <- coeffi3_plot %>% select(on_covid, big5, type,Estimate)
diff_plot <- {}

coeffi3_plot2 <- coeffi3_plot2[order(coeffi3_plot2$type),]
# coeffi3_plot2 %>% filter(big5 == 0)
# coeffi3_plot2
(coeffi3_plot2 %>% filter(big5 == 1))$Estimate[c(2,4,6)] - (coeffi3_plot2 %>% filter(big5 == 1))$Estimate[c(1,3,5)]
(coeffi3_plot2 %>% filter(big5 == 0))$Estimate[c(2,4,6,8)] - (coeffi3_plot2 %>% filter(big5 == 0))$Estimate[c(1,3,5,7)]

#--------------------------------------------
diff_plot$type = coeffi3_plot2$type %>% unique %>% data.frame()
# diff_plot <- diff_plot %>% rbind(diff_plot) 
diff_plot$dif_big5 <- c(79.361,0, -0.897,  1.523)
diff_plot$dif_non5 <- c(-8.172, 13.842, -1.635, -4.448)
diff_plot <- diff_plot %>% data.frame()
names(diff_plot)[1] = "type"
#--------------------------------------------
# draft 8 things,
diff_plot$type = coeffi3_plot2$type %>% unique %>% data.frame()
# diff_plot <- diff_plot %>% rbind(diff_plot) 
diff_plot$dif_big5 <- c(-27.997, 0, -2.823, -0.902)
diff_plot$dif_non5 <- c(-8.172, 13.842, -1.635, -4.448)
diff_plot <- diff_plot %>% data.frame()
names(diff_plot)[1] = "type"
...
#--------------------------------------------
diff_plot <- diff_plot %>% melt
diff_plot
# diff_plot <- mutate(diff_plot, ownertype = ifelse(variable == "dif_big5", ))
p_coef_dif <- ggplot(diff_plot) + geom_col(position = "dodge") +
  aes(x = type, y = value, fill = variable, group = variable) +
  geom_text(aes(x = type, y = value, fill = variable, group = variable, label = value),
  ...
  theme(text = element_text(size =20),
        plot.title = element_text(size = 25),
        axis.text.x = element_text(angle = 0)) + 
  labs(title  = "Coefficients difference between pre-COVID and post-COVID time",
       x = "Type of certain plants",
       y = "Estimated electricity generation amount")
  
 # p_coef 
p_coef_dif  
ggsave(filename = "../report/p_coef_dif_d8.png", plot = p_coef_dif, width = 12)
```
...
```{r}
pp_data_before <- pp_data_2 %>% filter(year != 20 | (year == 20 & month == 1))


pp_data_after <- pp_data_2 %>% filter(year == 20 & month != 1)
...
#--------------------------------------------
# draft 8
max = apply(pp_data_before3[,c(6,7)], 2, max)
min = apply(pp_data_before3[,c(6,7)], 2, min)
pp_data_before3 %>% names
scaled = as.data.frame(scale(pp_data_before3[,c(6,7)], center = min, scale = max - min))
#--------------------------------------------
#--------------------------------------------
...
#--------------------------------------------
# draft 8
pp_data_before3 <- pp_data_before3[,!names(pp_data_before3) %in% c("yearmon","area3","plant","generated_el" ,
                                                                   "capacity","complete",
                                                                   "time","op_time",
                                                                    "type_of_p","yearmonth","city",
                                                                   "bf_covid","af_covid")]
pp_data_before3 <- cbind(scaled, pp_data_before3)
names(pp_data_before3)[11:12] = c("fossilfuel", "bigfive")


max2 = apply(pp_data_after[,c(7,6)], 2, max)
min2 = apply(pp_data_after[,c(7,6)], 2, min)
scaled2 = as.data.frame(scale(pp_data_after[,c(7,6)], center = min2, scale = max2 - min2))

pp_data_after <- pp_data_after[,!names(pp_data_after) %in% c("yearmon","area3","plant","generated_el" ,
                                                                   "capacity","complete",
                                                                   "time","op_time",
                                                                    "type_of_p","yearmonth","city",
                                                                   "bf_covid","af_covid")]
...
#--------------------------------------------
# 11.6 预先 filter pp-data-after, 使得其名字和 before 一致
# 这样不行..............问题其实出现在train set & test set 分组. 有缺失把.
# setdiff(names(pp_train), names(pp_test))
# setdiff(names(pp_test), names(pp_train))
# 也没有阿

# setdiff(pp_data_after$plant_1, pp_data_before3$plant_1)
# 居然一样阿
#--------------------------------------------

# which(pp_data_before3$plant_1 == "yangguang#1-3ji")
```
#------------------------------------------
...
#------------------------------------------
## RMSE test data
plot(pp_test2$complete2, predict_testNN, col='blue', pch=16, ylab = "predicted complete NN", xlab = "real complete"); abline(0,1)

RMSE.NN = (sum((pp_test2$complete2 - predict_testNN)^2) / nrow(pp_test)) ^ 0.5; RMSE.NN

RMSE.NN0 = (sum((pp_test$complete - predict_testNN$net.result)^2) / nrow(pp_test)) ^ 0.5; RMSE.NN0
```


```{r}
# modify pp_data_after_dummy, for extra names of plants. not sure why appeared?
# 并不存在 after 有的 before 无的电厂. 已经证明.
# b = setdiff(names(pp_data_after_dummy), names(pp_test));b
# b
# 
# #--------------------------------------------
# # 下面这行可能有问题
# a = which(pp_data_after_dummy[,setdiff(names(pp_data_after_dummy), names(pp_test))] == 1);a
# # setdiff( names(pp_test),names(pp_data_after_dummy)) # yi bai duo ges
# # pp_data_after_dummy[,setdiff(names(pp_data_after_dummy), names(pp_test))] %>% View()
# # View(b)
# 
...
################## here u gotta compare pp_after and pp_after_dummmy observations. if equal, go compute!
#--------------------------------------------
# # modift pp_data_after
# dim(pp_test)
# # mmmodify <- gsub("parent_company_", "", names(pp_data_after_dummy));mmmodify
# mmmmodift <- gsub("plant_1_", "", b);mmmmodift 
...
# setdiff(names(pp_data_after), names(pp_test)) #0
c = setdiff(names(pp_data_bf_dummy), names(pp_data_after_dummy));c #135

add.dataframe = data.frame(c) %>% transpose()
names(add.dataframe) = add.dataframe[1,]
add.dataframe[1:1796, ] = as.numeric(0)
...
#save image of after dummy
#--------------------------------------------
# pp_data_after_dummy %>% dim
# for (i in 1:1791){#
#   for (j in 1:708){#
#     if (is.numeric(pp_data_after_dummy[i,j]) == F){
#       pp_data_after_dummy[i,j] <- as.numeric(pp_data_after_dummy[i,j])
#     }
#     # else{
#     #   print("no")
#     # }
#   }
#   }
pp_data_after_dummy3 <- lapply(pp_data_after_dummy, function(x) {
    if(is.character(x)) as.numeric(x) else x
})
pp_data_after_dummy3 <- pp_data_after_dummy3 %>% data.frame()

pp_data_after_dummy3$month_1 %>% class
# pp_data_after_dummy3 <- lapply(pp_data_after_dummy, function(x) {
    # if(is.factor(x)) as.numeric(as.character(x)) else x
# })


lapply(pp_data_after_dummy, function(x) {
    if(is.character(x)) print(x)
})


df2 <- mutate_all(pp_data_after_dummy, function(x) as.numeric(as.character(x)))
```
...
<!-- #  -->
<!-- # pp_data_after_dummy2 <- pp_data_after_dummy -->
<!-- # pp_20_1 <- pp_data_bf_dummy %>% filter(pp_data_bf_dummy$year_20 == 1) -->
<!-- # # dim(pp_20_1) -->
<!-- # names(pp_20_1) <- make.names(names(pp_20_1)) -->
<!-- # pp_data_after_dummy3 <- rbind(pp_20_1, pp_data_after_dummy) -->
<!-- # # pp_data_after_dummy %>% dim -->
<!-- # # which(pp_data_after_dummy3$month_10 == 1)  -->
<!-- # pp_data_after_dummy2 <- pp_data_after_dummy3 -->
<!-- #  -->
<!-- #  -->
<!-- #  -->
<!-- # prefix <- "month" %>% paste0("_", c(1:6)) -->
<!-- # prefix2 <- "month" %>% paste0("_", c(7:12)) -->
<!-- # pp_data_after_dummy2[,prefix2] = pp_data_after_dummy3[,prefix] -->
<!-- # pp_data_after_dummy2[,prefix] = 0 %>% as.numeric() -->
<!-- # pp_data_after_dummy4 = rbind(pp_data_after_dummy2, pp_data_after_dummy3) -->
<!-- # # rowsum(pp_data_after_dummy4[, c(prefix, prefix2)]C) -->
<!-- # #-------------------------------------------- -->
...
```{r, warning =F, message=F}
names(pp_data_after_dummy) <- make.names(names(pp_data_after_dummy))
# setdiff( pp_data_after_dummy, pp_test)
predict_after = compute(nn, df2[,-2])
...
# plot(nn)
# pp_test <- pp_test[ , names(pp_test) %in% n]
predict_after$net.result %>% summary
pp_test2$complete %>% summary
#--------------------------------------------
# debug
# setdiff(pp_data_after_dummy3 ,pp_test)
# error(setdiff(pp_data_after_dummy ,pp_test))
```
## rp result.
...
# predict_19_20_after = compute(nn, pp_data_after_dummy4[,-2])
...
# 
# for (i in 1:dim(pp_data_after_dummy4)[1]){
#   for (j in 8:19){
#     if (pp_data_after_dummy4[i,j] == 1){
#       pp_data_after_dummy4$month[i] = gsub("month_","",names(pp_data_after_dummy4)[j])
#     }
#   }
# }
# 
# # which(is.na(pp_data_after_dummy4$month) == F)
# # pp_data_after_dummy4$month %>% unique
# pp_data[which(pp_data$plant_1 == "wuxiangdianchang"),] %>% View()
# #--------------------------------------------
#--------------------------------------------


data_funny <- cbind(pp_data_after, predict_afternn)
setdiff(pp_data_after$plant_1, pp_data_before3$plant_1)

data_funny$month <- data_funny$month %>% as.numeric()
#--------------------------------------------
# ma de fen ceng chou yang


# strata(data_funny, "month", size = )
#--------------------------------------------

# predict_19_20_afternn %>% range
# data_19 <- pp_data %>% filter(year == 17)
# data_19 <- cbind(predict_19_20_afternn, data_19)
# data_19$month <- data_19$month %>% as.numeric()
dfpp <- data_funny %>% group_by(month) %>% summarise(sum(predict_afternn))

dfpp
dfpp %>% names()
gpt(dfpp) + aes(x = month, y = `sum(predict_afternn)`) +
  geom_line() +
...
# mynnetfit <- train(DOC  ~ ., data = MyData, method = "nnet", maxit = 100, tuneGrid = my.grid, trace = f)
...
```{r}
plot(pp_test_lr$complete, pred_test_lr, col='blue', pch=16, ylab = "predicted complete NN", xlab = "real complete"); abline(0,1)

RMSE.NN = (sum((pp_test_lr$complete - pred_test_lr)^2) / nrow(pp_test_lr)) ^ 0.5; RMSE.NN
```

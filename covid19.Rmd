...
```{r}
pp_data_bf_dummy <- fastDummies::dummy_cols(pp_data_before3, select_columns = c("month","parent_company", "area", "plant_1" ))

pp_data_bf_dummy <- pp_data_bf_dummy[,!names(pp_data_bf_dummy) %in% c("month","parent_company", "area", "plant_1" )]
#------------------------------------------
pp_data_after_dummy <- fastDummies::dummy_cols(pp_data_after, select_columns = c("month","parent_company", "area", "plant_1" ))

pp_data_after_dummy <- pp_data_after_dummy[,!names(pp_data_after_dummy) %in% c("month","parent_company", "area", "plant_1" )]
```
...
pp_test$complete2 <- (pp_test$complete * (max(pp_data_before$complete) - min(pp_data_before$complete))) + min(pp_data_before$complete)
```
...
#------------------------------------------
## RMSE test data
plot(pp_test$complete2, predict_testNN, col='blue', pch=16, ylab = "predicted rating NN", xlab = "real rating")
